...
* `version: '2.8.0'` — The MacPorts version to install.
...
jobs:
  install-macports-on-macos-11:
    runs-on: macos-11
    name: 'Install MacPorts 2.8.0 on MacOS 11'
    steps:
      ...
      - name: 'Validate installed MacPorts version'
        run: >-
          test "$(port version)" = 'Version: 2.8.0'
      ...
      - name: 'Validate transmitted MacPorts version'
        run: >-
          test "${{ steps.macports.outputs.version }}" = '2.8.0'

  install-macports-on-macos-12:
    runs-on: macos-12
    name: 'Install MacPorts 2.8.0 on MacOS 12'
    steps:
      - uses: actions/checkout@v3
      - name: 'Run testsuite'
